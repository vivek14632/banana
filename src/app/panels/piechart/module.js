...
function (angular, app, _, $, kbn) {
  ...
  module.controller('piechart', function($scope, $timeout, timer, querySrv, dashboard, filterSrv) {
    $scope.panelMeta = {
      modals : [
        {
          description: "Inspect",
    ...
    $scope.build_query = function(filetype, isForExport) {
      ...
      var rows_limit = isForExport ? '&rows=0' : ''; // for piechart, we do not need the actual response doc, so set rows=0
      var facet = '';
    ...
    $scope.exportfile = function(filetype) {
      ...
      response.then(function(response) {
        kbn.download_response(response, filetype, "piechart");
      });
    };
    ...
    $scope.get_data = function() {
      ...
      results.then(function(results) {
        ...
        $scope.data.push({label:'Missing field',
          // data:[[k,results.facets.piechart.missing]],meta:"missing",color:'#aaa',opacity:0});
        ...
        $scope.data.push({label:'Other values',
          // data:[[k+1,results.facets.piechart.other]],meta:"other",color:'#444'});
    ...
    $scope.build_search = function(term,negate) {
      if(_.isUndefined(term.meta)) {
        filterSrv.set({type:'piechart',field:$scope.panel.field,value:term.label,
  ...
  module.directive('piechartChart', function(querySrv,dashboard,filterSrv) {
    return {
      restrict: 'A',
      link: function(scope, elem) {
        ...
        function render_panel() {
          ...
          if (filterSrv.idsByTypeAndField('piechart',scope.panel.field).length > 0) {
